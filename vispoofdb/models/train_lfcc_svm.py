# File: vispoofdb/models/train_lfcc_svm.py
import numpy as np
import os
import sys
import joblib
...
LOAD_DIR = BASE_DIR / 'vispoofdb' / 'data' / 'features_lfcc'
SAVE_MODEL_DIR = BASE_DIR / 'vispoofdb' / 'models_saved'
SAVE_MODEL_DIR.mkdir(parents=True, exist_ok=True)

# --- 1. Load dữ liệu LFCC ---
print("Đang tải dữ liệu LFCC...")
X      = np.load(LOAD_DIR / 'X_lfcc.npy')
y      = np.load(LOAD_DIR / 'y_lfcc.npy')
splits = np.load(LOAD_DIR / 'splits_lfcc.npy', allow_pickle=True)

print(f"Tổng số mẫu: {len(X)}")
print(f"Phân phối splits: {dict(zip(*np.unique(splits, return_counts=True)))}")

# --- 2. Filter theo splits (KHÔNG dùng train_test_split ngẫu nhiên) ---
...
# --- 6. Lưu mô hình ---
joblib.dump(model,  SAVE_MODEL_DIR / 'svm_lfcc_model.pkl')
joblib.dump(scaler, SAVE_MODEL_DIR / 'scaler_lfcc.pkl')
